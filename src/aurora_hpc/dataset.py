...
class AuroraDataset(Dataset):
    ...
    def _get_batch(self, timerange):
        ...
        """
        # PyTorch doesn't support non-writable arrays.
        surf_vars_ds = self.surf_vars_ds
        # surf_vars_ds.setflags(write=True)

        # Todo: finish this:
        #  make a writeable copy of each numpy array
        return Batch(
            surf_vars={
                # First select time points `index` and `index - 1`. Afterwards, `[None]` inserts a
                # batch dimension of size one.
                "2t": torch.from_numpy(
                    surf_vars_ds["t2m"].values[timerange][None].copy()
                ),
                "10u": torch.from_numpy(
                    surf_vars_ds["u10"].values[timerange][None].copy()
                ),
                "10v": torch.from_numpy(
                    surf_vars_ds["v10"].values[timerange][None].copy()
                ),
                "msl": torch.from_numpy(
                    surf_vars_ds["msl"].values[timerange][None].copy()
                ),
            },
            static_vars={
                # The static variables are constant, so we just get them for the first time.
                "z": torch.from_numpy(self.static_vars_ds["z"].values[0].copy()),
                "slt": torch.from_numpy(self.static_vars_ds["slt"].values[0].copy()),
                "lsm": torch.from_numpy(self.static_vars_ds["lsm"].values[0].copy()),
            },
            atmos_vars={
                "t": torch.from_numpy(
                    self.atmos_vars_ds["t"].values[timerange][None].copy()
                ),
                "u": torch.from_numpy(
                    self.atmos_vars_ds["u"].values[timerange][None].copy()
                ),
                "v": torch.from_numpy(
                    self.atmos_vars_ds["v"].values[timerange][None].copy()
                ),
                "q": torch.from_numpy(
                    self.atmos_vars_ds["q"].values[timerange][None].copy()
                ),
                "z": torch.from_numpy(
                    self.atmos_vars_ds["z"].values[timerange][None].copy()
                ),
            },
            metadata=Metadata(
                lat=torch.from_numpy(self.surf_vars_ds.latitude.values.copy()),
                lon=torch.from_numpy(self.surf_vars_ds.longitude.values.copy()),
